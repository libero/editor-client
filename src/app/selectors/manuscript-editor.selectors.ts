...
export const isExportTaskRunning = createSelector(
  getManuscriptEditorState,
  ({ exportTask }) => exportTask && exportTask.status === PDF_TASK_STATUSES.PDF_EXPORT_RUNNING
);

export const isExportTaskFailed = createSelector(
  getManuscriptEditorState,
  ({ exportTask }) => exportTask && exportTask.status === PDF_TASK_STATUSES.PDF_EXPORT_ERROR
);

export const isExportTaskFinished = createSelector(
  getManuscriptEditorState,
  ({ exportTask }) => exportTask && exportTask.status === PDF_TASK_STATUSES.PDF_EXPORT_SUCCESS
);
